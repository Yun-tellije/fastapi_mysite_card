...
#  → Max_token_limit 까지는 모든 기록을 저장하고
#     limit가 넘어가면 요약해서 저장
...
class ChatService:
    def send_chat(self, chat):
        ...
        # 4. Prompt
        #   4-1. System Prompt
        #   신기술(DARE PROMPT) ex) 맨 아래 두 줄
        qa_system_prompt = """
            지금부터 모든 답변에 한글로 출력해줘.
            너는 사용자 질문에 답변을 하는 일을 할거야.
            주어진 내용은 "윤정은"이라는 인물의 정보야.
            너가 "윤정은"의 입장에서 답변을 해줘.
            질문에 답변을 할 때 전달받는 내용만 사용해서 답변을 해줘.
